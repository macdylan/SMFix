...
// nz hides negative "unknown" defaults (e.g. missing filament stats)
...
func headerV0() [][]byte {
	...
	h = append(h, H(";Filament used: %.5fm", nz(Params.AllFilamentUsed())/1000.0))
	...
	totalLinesIdx := len(h)
	h = append(h, nil) // placeholder: ;file_total_lines, filled below
	...
	h = append(h, H(";build_plate_temperature(°C): %.0f", nz(Params.EffectiveBedTemperature())))
	...
	h = append(h, H(";matierial_weight: %.4f", nz(Params.AllFilamentUsedWeight())))
	h = append(h, H(";matierial_length: %.5f", nz(Params.AllFilamentUsed())/1000.0))
	...
	// ;Header End carries a trailing "\n\n": the emitted header occupies
	// len(h)+1 physical lines, plus Params.TotalLines gcode lines.
	h[totalLinesIdx] = H(";file_total_lines: %d", Params.TotalLines+len(h)+1)
	return h
}
...
func headerV1() [][]byte {
	...
	totalLinesIdx := len(h)
	h = append(h, nil) // placeholder: ;Lines, filled below
	...
	h = append(h, H(";Extruder 1 Retraction Distance:%.2f", Params.Retractions[1]))
	h = append(h, H(";Extruder 1 Switch Retraction Distance:%.2f", Params.SwitchRetraction[1]))
	h = append(h, H(";Bed Temperature:%.0f", nz(Params.EffectiveBedTemperature())))
	...
	// ;Header End carries a trailing "\n\n": the emitted header occupies
	// len(h)+1 physical lines, plus Params.TotalLines gcode lines.
	h[totalLinesIdx] = H(";Lines:%d", Params.TotalLines+len(h)+1)
	return h
}
